...
function unitCardHTML(unit, actions, viewType="unit", opts={}) {
  ...
  const standsHTML = viewType === "unit"
    ? `<span style="font-size:11px;color:var(--text-muted);flex-shrink:0;white-space:nowrap">${pts.unitSize} stand${pts.unitSize>1?"s":""}</span>`
    : "";
  return `
    <div class="unit-card">
      ...
      <div class="unit-card-traits"><span class="unit-card-section-label">Stand Traits</span>${traitsHTML}</div>
      ${weaponsHTML}
...
function mechPairCardHTML(infUnit, tuUnit, n, actionsHTML, viewType, opts={}) {
  function halfInner(unit, vt, extra={}) {
    ...
    const topRightHTML = extra.countBadge != null
      ...
      : (vt === "unit" ? `<span style="font-size:11px;color:var(--text-muted);flex-shrink:0;white-space:nowrap">${pts.unitSize} stand${pts.unitSize>1?"s":""}</span>` : "");
...
function openTraitPicker(target, wIdx) {
  ...
  document.getElementById("trait-modal-title").textContent = target==="stand" ? "Stand Traits" : `Weapon ${wIdx+1} Traits`;
...
function calculateBuilder() {
  ...
  const roleTiles = [
    {key:"unit",    label:`Unit (${pts.unitSize} stand${pts.unitSize>1?"s":""})`, pts:pts.unitPts},
  ...
  const bdHTML = bd ? (() => {
    ...
    return `
      ...
      <div class="summary-row"><span>Per stand</span><span>${pts.perStand} pts</span></div>
      ${typeRow}`;
  })() : "";
...
function _openTransportPicker(unit, current, faction) {
  ...
  intro.innerHTML = `<strong style="color:#e0e0e0">${esc(unit.name)}</strong> - ${stands} ${noun} stand${stands!==1?"s":""} needing <strong>${need}</strong> transport slot${need!==1?"s":""}${singleNote}. Pick a transport AFV; the minimum number needed is added automatically.`;
...
function _pUnitCard(unit, viewType, tfBadgeHTML, opts, pal, gray) {
  ...
  const unitSizeStr = opts.tag ? "" : (viewType === "unit" ? ` <span style="font-size:10px;color:${pal.mutedText};font-weight:normal">${pts.unitSize} stand${pts.unitSize>1?"s":""}</span>` : "");
  ...
  return `<div style="background:${pal.cardBg};border:1.5px solid ${pal.cardBorder};border-radius:8px;padding:10px;break-inside:avoid;page-break-inside:avoid">
    ...
    <div style="margin-bottom:5px">
      <div style="font-size:8px;text-transform:uppercase;letter-spacing:.05em;color:${pal.mutedText};font-weight:700;margin-bottom:3px">Stand Traits</div>
...
function _pMechPair(carrier, viewType, tu, n, badgeHTML, pal, gray, carrierPtsVal) {
  ...
  function halfContent(unit, vt, extra) {
    ...
    const countHTML = extra.countBadge != null
      ? `<div style="margin-top:5px;font-size:14px;font-weight:bold;color:${connFg}">&times;${extra.countBadge}<span style="font-size:9px;font-weight:normal;color:${pal.mutedText}"> stands</span></div>`
      : "";
    const unitSizeStr = extra.countBadge == null && vt === "unit"
      ? ` <span style="font-size:10px;color:${pal.mutedText};font-weight:normal">${pts.unitSize} stand${pts.unitSize>1?"s":""}</span>`
      : "";
  ...
  return `<div class="mech-pair-print" style="display:flex;background:${pal.cardBg};border:1.5px solid ${pal.cardBorder};border-radius:8px;overflow:hidden;break-inside:avoid;page-break-inside:avoid">
    ...
    ${halfContent(tu, "unit", {countBadge: n, tag: "Transport", ptsVal: tuPtsPerStand, ptsLabel: "/ Stand"})}
  </div>`;
}
  ...
  listEl.innerHTML = filtered.map(u => {
    ...
    return `<div class="list-row" style="flex-direction:column;align-items:stretch;gap:6px;cursor:default">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
        <span style="font-size:11px;color:#8b949e">${u.speed}" ${u.mobility} &bull; Aim ${u.aim}+ &bull; Save ${u.saveNumber}+ &bull; ${pts.unitSize} stand${pts.unitSize!==1?"s":""}</span>
